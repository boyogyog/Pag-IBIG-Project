...
public class MemberRecordForm extends JFrame {
    ...
    // FIX: renamed to avoid shadowing; always populated from constructor
    ...
    // =========================================================================
    // FIX: Primary constructor — loggedInMID must always be passed here.
    // If you call new MemberRecordForm(caller) without MID, use the overload below.
    // =========================================================================
    public MemberRecordForm(JFrame caller, String loggedInMID) {
        this.caller      = caller;
        // FIX: guard against null so loadAllData doesn't silently skip
        ...
        backBtn.addActionListener(e -> {
            ...
            if (choice == JOptionPane.YES_OPTION) {
                // FIX: pass loggedInMID back to SignInFrame so session is preserved
        ...
        // Load all data after UI is built
        loadAllData();
    }
    ...
    private void loadAllData() {
        // FIX: guard — if MID is blank, show a warning and abort (don't silently skip)
        if (loggedInMID == null || loggedInMID.isEmpty()) {
            showError("No logged-in MID available. Please log in again.");
            return;
        }

        // FIX: pre-fill the MID field so it always shows
        ...
        if (m != null) {
            ...
            // FIX: warn if member row not found for the given MID
    ...
    private void handleSaveAll() {
        ...
        if (!"Select".equals(curCompanyBox.getSelectedItem()) &&
            ...
            Date dateEmp = null;
            try { dateEmp = Date.valueOf(curDateEmpField.getText().trim()); }
        ...
        for (PrevEmpEntry entry : prevEmpEntries) {
            ...
            String from = entry.fromField.getText().trim();
            String to   = entry.toField.getText().trim();
            Date fromDate = from.isEmpty() ? null : Date.valueOf(from);
            Date toDate   = to.isEmpty()   ? null : Date.valueOf(to);
        ...
        for (HeirEntryPanel entry : heirEntries) {
            ...
            Date birthDate = dob.isEmpty() ? null : Date.valueOf(dob);
    ...
    private JPanel buildContent() {
        ...
        JPanel r15 = row(2);
        // FIX: show actual loggedInMID in the read-only MID field in current employment section
        ...
        r16.add(lf("Date Employed (YYYY-MM-DD)", curDateEmpField    = tf("")));
    ...
    // FIX: enhanced setCombo with case-insensitive matching
    private void setCombo(JComboBox<String> box, String value) {
        ...
        // partial / normalised match (strips spaces, checks contains)
